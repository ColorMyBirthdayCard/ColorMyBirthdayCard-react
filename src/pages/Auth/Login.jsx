...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const { data: { userId, sessionId, userName, userBirthday } } = await AuthApi.signIn(userInfo);
      dispatch({
        type: 'LOGIN', memberId: userId, userName: '김영균', userBirthday: '1998-12-30'
      });
      window.sessionStorage.setItem('sessionId', sessionId);
      window.sessionStorage.setItem('memberId', userId);
      window.sessionStorage.setItem('name', userName);
      window.sessionStorage.setItem('birthday', userBirthday);
